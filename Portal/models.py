...
class Post(models.Model):
    post_ID = models.BigAutoField(primary_key=True, verbose_name='ID',unique=True)
    author_ID_FK = models.ForeignKey('Author', db_column='author_ID_FK', to_field='user_id',
                                        on_delete=models.PROTECT,db_constraint=True,verbose_name='Автор ')
    post_type=models.CharField(max_length=4, choices=POST_TYPE,default=article, verbose_name='Тип статьи')
    date_and_time_created = models.DateField(auto_now_add=True, verbose_name='Дата и время создания')
    category_ID_FK=models.ManyToManyField(Category,through='PostCategory', verbose_name='Категория')
